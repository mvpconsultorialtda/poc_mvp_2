...
const BackgroundRemover: React.FC<BackgroundRemoverProps> = ({ file, onClose }) => {
  ...
  useEffect(() => {
    ...
    const process = async () => {
      try {
        ...
        if (!isCancelled) toast.error(`Falha ao processar a imagem: ${e.message}`);
      } finally {
  ...
  const ToolButton: React.FC<{ toolName: 'brush' | 'eraser'; icon: React.ReactNode }> = ({ toolName, icon }) => (
    <button onClick={() => setActiveTool(prev => prev === toolName ? null : toolName)} className={`p-2 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${activeTool === toolName ? 'bg-sky-500 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-300 dark:hover:bg-slate-600'}`} disabled={!processedImageUrl || isLoading} aria-label={toolName} aria-pressed={activeTool === toolName}>
      {icon}
    </button>
  );
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-6xl max-h-[90vh] flex flex-col shadow-2xl animate-scale-up">
          ...
          <div className="p-4 border-y border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row items-center justify-center gap-4 flex-shrink-0">
              ...
              <div className="flex items-center space-x-3 w-full sm:w-auto" aria-label="Tamanho do pincel">
                  ...
                  <input type="range" min="5" max="100" value={brushSize} onChange={(e) => setBrushSize(parseInt(e.target.value))} className="w-32 accent-sky-600" disabled={!processedImageUrl || isLoading} />
              </div>
          </div>
          ...
          <footer className="flex justify-between items-center p-4 bg-slate-50 dark:bg-slate-800/50 flex-shrink-0">
            ...
            <button onClick={handleDownload} disabled={!processedImageUrl || isLoading} className="px-6 py-2 rounded-lg bg-sky-600 text-white hover:bg-sky-700 font-semibold flex items-center space-x-2 disabled:bg-slate-400 dark:disabled:bg-slate-600 disabled:cursor-not-allowed">
